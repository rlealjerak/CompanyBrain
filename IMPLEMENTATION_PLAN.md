...
3. Branch on what the SELECT returned:
   ...
   - **`pending` row found:** Verify its `ingestion_job_id` is **not** the current job's `job_id` (if it matches, something is structurally wrong — rollback, surface an error). Claim the row in-place: `UPDATE documents SET ingestion_job_id=:current_job_id WHERE id=:stale_doc_id AND ingestion_job_id=:stale_job_id`. Because Commit 2 is fully atomic, a `pending` row that survived a crash is guaranteed to have no children — Commit 2 either committed all chunks/embeddings together or rolled back entirely, leaving nothing to clean up. Skip step 4 and proceed directly to step 5 (COMMIT). **Do not DELETE and re-INSERT the same `(source_id, content_hash)` key in one transaction** — PostgreSQL non-deferrable unique constraints are checked per-statement, and while DELETE-then-INSERT of the same key within one transaction is technically safe, it is fragile and implementation-dependent; UPDATE-in-place avoids the question entirely.
   - **No row found:** Proceed to step 4.
4. *(Only reached on the "no row" path.)* `INSERT INTO documents (..., ingestion_status='pending', ingestion_job_id=:current_job_id)`. If the INSERT fails with a unique constraint violation — meaning a concurrent job committed its own row between step 1's `FOR UPDATE` returning nothing and this INSERT — treat it as "another job claimed this file version, abort cleanly." This is not a retryable error.
...
If the worker crashes between Commit 1 and Commit 2, the `pending` row persists with no children (Commit 2 is atomic — it either fully committed or fully rolled back, leaving no orphaned chunks or embeddings). The next run's Commit 1 transaction acquires the `FOR UPDATE` lock on the pending row, detects it's stale, and claims it in-place via `UPDATE ingestion_job_id=:new_job_id` — no delete-and-reinsert of the same unique key, no constraint risk. If the worker crashes during Commit 2, the transaction rolls back — the `pending` row from Commit 1 remains, and the same repair path applies on the next run.
...
### Idempotency
- Every ingestion task performs an atomic document-claim transaction (Commit 1) that combines the hash check, optional stale pending-row repair, and pending row ownership into one `FOR UPDATE`-serialized transaction. A `complete` match means skip. A `pending` match owned by a stale job is claimed in-place via `UPDATE ingestion_job_id=:current_job_id` — no DELETE-then-INSERT of the same unique key. A unique constraint violation on INSERT (no-row path) means a concurrent job claimed the slot — abort cleanly. Because Commit 2 is atomic, a pending row under repair is guaranteed to have no orphaned children requiring cascade cleanup.
...
| Two concurrent jobs race through Commit 1 for the same source, one failing on UNIQUE constraint | Low | High | `SELECT FOR UPDATE` on existing row serializes concurrent attempts; pending-row repair uses UPDATE-in-place (no same-key DELETE-then-INSERT); unique constraint violation on INSERT (no-row race) is caught and treated as clean abort, not a job failure |
