...
- `embeddings` — vectors linked to chunks, with source metadata and two timestamps: `source_timestamp` (the source document's own creation or last-modified time, copied from `Document.timestamp` at embed time — used for recency-based reranking so that an older source ingested after a newer source still ranks below it) and `created_at` (ingestion time — audit and operational use only, never used for reranking). The vector column must be declared as dimension **1024** to match the `voyage-3` output. Setting the wrong dimension here breaks all vector operations and requires a migration to fix.
...
- `tasks` — for the PIL: description, user ID, `urgency_score FLOAT` (range 1–10; Phase 5 clamps to this range), `urgency_band TEXT` (values: `high`, `medium`, `low` — matching Phase 5's bands: high = 8–10, medium = 5–7, low = 1–4), `status TEXT` (values: `open`, `complete`, `snoozed`, `dismissed` — these are the exact four states the Phase 5 PATCH endpoint and dashboard action buttons use; do not use `in_progress` or `done`), deadline, context bundle (JSONB), `source_reference TEXT NOT NULL` — a namespaced identifier for the source message container that generated the task (format: `slack:{thread_ts}`, `email:{message_id}`, `ticket:{ticket_id}`), and `task_fingerprint TEXT NOT NULL` — a SHA-256 hash derived from `source_reference` concatenated with the normalized task description, uniquely identifying one extracted task within its source container. One Slack thread or email chain can produce multiple tasks for the same user; `source_reference` alone cannot distinguish them. Both fields must be populated before every INSERT. Add `UNIQUE (user_id, source_reference, task_fingerprint)` — not `UNIQUE (user_id, source_reference)` — so multiple distinct tasks from the same source are allowed while duplicate extraction of the same task is blocked across retries and repeated scans.
...
**3.5 — Embedder**  
Implement the embedder in `app/pipeline/embedder.py` using the **Voyage AI SDK** (`voyageai` Python package), not the Anthropic SDK. Anthropic's Claude API is generation-only and has no embeddings endpoint. The embedding model is `voyage-3`, which produces 1024-dimension vectors. Batch API calls — the Voyage AI SDK supports batching up to 128 texts per request, which is dramatically faster than one request per chunk. Store the resulting vectors alongside the chunk's metadata: source, source type, document ID, chunk index, `source_timestamp` (copied from `Document.timestamp` — the source document's own creation or last-modified time), and `created_at` (set to `now()` at embed time — ingestion time for audit purposes only). The `source_timestamp` is the recency signal used in Phase 4's reranking step to resolve contradictions: an older source file ingested after a newer one must still rank below the newer one. Never rerank on `created_at`.
...
3. If stale: execute these two statements **in this exact order** within the same open transaction:
   - `UPDATE ingestion_jobs SET status='failed' WHERE id=:existing_id`
   - `INSERT INTO ingestion_jobs (..., status='running', locked_at=now(), attempt_number=1)`
   - COMMIT
   
   **Why this works against a non-deferrable partial unique index:** PostgreSQL evaluates non-deferrable unique constraint violations at the end of each individual statement, not at end-of-transaction. When the UPDATE executes, it changes the old row's status to `'failed'`, which removes it from the partial index predicate (`WHERE status IN ('running','retrying')`). By the time the INSERT fires and its constraint check runs, the old row is already out of the partial index scope — no conflict exists. This does not require a deferrable constraint. The order is load-bearing: INSERT before UPDATE would fail because the old row is still in the partial index scope when the INSERT's constraint check runs. Never reverse the order, and never split these two statements into separate transactions — splitting them leaves the source slot unguarded between retirement and replacement.
...
Before ingesting a file, compute a SHA-256 hash of its raw content. The hash check and the resulting document row insertion are performed together in a single **atomic document-claim transaction** (this is Commit 1). The claim must be atomic even when no prior row exists — `SELECT FOR UPDATE` only locks rows that exist, so a plain SELECT-then-INSERT leaves a window where two concurrent fresh jobs both find no row, both proceed, and one fails on the UNIQUE constraint after potentially advancing in the algorithm. The fix is to attempt the INSERT first: if it succeeds, the claim is atomic; if it conflicts, a row already exists and `SELECT FOR UPDATE` can then lock and inspect it safely.
...
2. Attempt atomic first claim: `INSERT INTO documents (..., ingestion_status='pending', ingestion_job_id=:current_job_id) ON CONFLICT (source_id, content_hash) DO NOTHING RETURNING id`.
   - **INSERT returned a row:** This job atomically owns the new `pending` row — no concurrent job can claim the same `(source_id, content_hash)`. Proceed to step 5 (COMMIT).
   - **INSERT returned nothing:** A row already exists. Proceed to step 3.
3. *(Only reached when INSERT conflicted.)* `SELECT id, ingestion_job_id, ingestion_status FROM documents WHERE source_id=:sid AND content_hash=:hash FOR UPDATE` — lock the existing row. `FOR UPDATE` now has a row to lock, serializing any other concurrent attempt that also reached step 3.
4. Re-run fencing check inside the same transaction. Branch on the locked row:
   - **`complete` row:** This file version is fully ingested — rollback, skip.
   - **`pending` row, `ingestion_job_id` matches current job:** This is a retry — the job already owns this pending row from a prior attempt. No claim action needed; rollback the Commit 1 transaction and proceed directly to the pipeline steps. The existing pending row is the correct starting point.
   - **`pending` row, stale `ingestion_job_id`:** Claim in-place: `UPDATE documents SET ingestion_job_id=:current_job_id WHERE id=:doc_id AND ingestion_job_id=:stale_job_id`. Because Commit 2 is fully atomic, the stale pending row has no children to clean up. Proceed to step 5. Never DELETE and re-INSERT the same `(source_id, content_hash)` key.
...
If the worker crashes between Commit 1 and Commit 2, the `pending` row persists with no children (Commit 2 is atomic — it either fully committed or fully rolled back). The next run's Commit 1 attempts `INSERT ... ON CONFLICT DO NOTHING`; the INSERT returns nothing (row exists), step 3 locks it with `SELECT FOR UPDATE`, step 4 finds `ingestion_job_id` belonging to the now-`failed` stale job, and claims it in-place via UPDATE — no delete-and-reinsert, no constraint risk. If the worker crashes during Commit 2, the transaction rolls back — the `pending` row from Commit 1 remains, and the same repair path applies on the next run. For retries (Algorithm B), Commit 1 finds `ingestion_job_id` matching the current job at step 4, recognizes the row as already owned, skips the claim, and proceeds directly to the pipeline.
...
3. **Rerank** the results by a combination of relevance score and `source_timestamp` — this ensures the newer refund policy ranks above the older one regardless of ingestion order. Never rerank on `created_at` (ingestion time).
...
Implement JWT signing and verification in `app/core/auth.py`. The JWT payload must contain only the `user_id` — do not embed the `role` in the token. Create two FastAPI dependencies: `get_current_user` (validates the token signature and expiry, then loads the full user record from the database using the `user_id` claim) and `require_admin` (calls `get_current_user` and raises 403 if the database-loaded user's role is not `admin`). Apply `get_current_user` to all non-auth endpoints. Apply `require_admin` to all ingestion endpoints.

Loading the user from the database on every request is required so that a revoked admin's token cannot continue to authorize ingestion after their role is downgraded. Trusting the role embedded in a long-lived token means role revocation is not effective until token expiry — on ingestion endpoints, which can drive unbounded Claude API spend, this is unacceptable. The database lookup is the authoritative source of role truth. Set access token TTL to 15 minutes to bound the worst-case window for any other stale claim.
...
- **Crash after document insert, before chunks complete:** Insert a `pending` document row owned by job_id=100 for a source, then run a fresh ingestion attempt (job_id=101); assert Commit 1 acquires `FOR UPDATE` on the pending row, claims it in-place via `UPDATE SET ingestion_job_id=101` without deleting it, the row's primary key is unchanged, Commit 2 then completes, and the document ends as `complete` with all expected chunks and embeddings — no DELETE of the pending row, no fresh INSERT of the same unique key
...
- **Transaction rollback leaves detectable `pending` row:** Commit the `pending` document row (Commit 1), then force a rollback of Commit 2 (simulate a DB error during chunk write); assert the `pending` row survives with its original primary key intact (it was committed in Commit 1 and Commit 2 rolled back without touching it); assert the next run's Commit 1 claims it in-place via `UPDATE SET ingestion_job_id=:new_job_id`, then Commit 2 completes and the document ends as `complete` — the row is never deleted and reinserted
...
- **Algorithm C integration test against real PostgreSQL schema:** Using the actual Alembic migration (not an in-memory or mocked schema), run Algorithm C's exact UPDATE-then-INSERT sequence inside one transaction against the live partial unique index; assert the transaction commits without a unique constraint violation; assert reversing the order (INSERT before UPDATE) raises a unique constraint violation — this proves the order dependency against the real DDL and prevents an implementer from accidentally swapping the statements
- **Stale takeover + pending-row repair while evicted worker resumes:** Simulate worker A (job_id=100) that commits a `pending` document row then is evicted by a stale takeover marking job 100 as `failed`; let worker B (job_id=101) enter Commit 1: acquire `SELECT FOR UPDATE` on the pending row, verify `ingestion_job_id=100` (not 101), claim it in-place via `UPDATE SET ingestion_job_id=101 WHERE id=:doc_id AND ingestion_job_id=100`, assert no DELETE occurs and the document primary key is unchanged; then resume worker A and attempt its next write checkpoint; assert worker A's fencing read detects `status='failed'` and aborts all writes without persisting any data; assert worker B completes Commit 2 with a `complete` document row whose primary key matches the original pending row and is linked to job 101
...
- **Pending-row repair claims in-place, never DELETE-then-INSERT same key:** Insert a `pending` document row owned by job_id=100; start a new job (job_id=101); assert Commit 1 acquires `FOR UPDATE` on the pending row; assert the repair is an `UPDATE SET ingestion_job_id=101` on the existing row, not a DELETE followed by INSERT of the same `(source_id, content_hash)` key; assert the row's primary key is unchanged after repair; assert no unique constraint violation occurs
- **Commit 1 document repair integration test against real PostgreSQL schema:** Using the actual Alembic migration (not mocked), run the full Commit 1 repair sequence — insert stale pending row, run UPDATE-in-place in a new transaction, assert commit succeeds, assert the row is owned by the new job and `ingestion_status` is still `pending`, assert no duplicate rows exist under the `UNIQUE (source_id, content_hash)` constraint
- **Concurrent first-ingestion of same file version:** Simulate two concurrent fresh jobs for the same `(source_id, content_hash)` with no prior document row; assert the `INSERT ... ON CONFLICT DO NOTHING` serializes them — exactly one returns a row ID (wins the atomic claim) and the other returns nothing; assert the loser reads the existing row via `SELECT FOR UPDATE`, finds a `pending` row owned by the winner's job, and aborts cleanly; assert exactly one `pending` document row exists and no unique constraint error surfaces as a job failure
- **Retry recognizes its own pending row:** Commit a `pending` row owned by job_id=100; simulate a retry invocation (Algorithm B, same job_id=100); assert Commit 1's `INSERT ON CONFLICT` returns nothing, the `SELECT FOR UPDATE` finds `ingestion_job_id=100` matching the current job, and the retry skips the claim and proceeds directly to the pipeline without modifying the document row
...
- The JWT payload carries only the `user_id`. The role is never embedded in the token. `get_current_user` loads the full user record from the database on every request; `require_admin` reads the role from that database record. This ensures role revocation takes effect immediately rather than waiting for token expiry.
- All non-auth endpoints require a valid JWT. Ingestion endpoints additionally require the `admin` role verified against the current database record. No exceptions.
...
### Idempotency
- Every ingestion task performs an atomic document-claim transaction (Commit 1). The claim uses `INSERT ... ON CONFLICT DO NOTHING RETURNING id` as the first step: if the INSERT wins, the job atomically owns the new pending row; if it conflicts, a row already exists and `SELECT FOR UPDATE` can then lock and inspect it. A `complete` match means skip. A `pending` match with a matching `ingestion_job_id` means this is a retry that already owns the row — skip the claim and proceed to the pipeline. A `pending` match owned by a stale job is claimed in-place via `UPDATE ingestion_job_id=:current_job_id` — no DELETE-then-INSERT. Because Commit 2 is atomic, a stale pending row has no orphaned children requiring cleanup.
...
- Database `UNIQUE` constraints on `documents (source_id, content_hash)` and `tasks (user_id, source_reference, task_fingerprint)` are the last line of defense against duplicates. Application-level checks reduce load; database constraints prevent corruption.
...
| Implementer splits Algorithm C UPDATE and INSERT into separate transactions, believing the non-deferrable partial unique index requires it | Low | High | Plan explicitly documents that PostgreSQL evaluates non-deferrable unique constraints per-statement: UPDATE removes old row from index scope before INSERT fires; order is load-bearing (UPDATE must precede INSERT); integration test proves correctness against the real schema |
...
| Crashed run leaves `pending` document that hash-skip treats as complete, silently dropping chunks/embeddings | Low | High | Hash skip checks `ingestion_status = 'complete'`; `pending` match triggers UPDATE-in-place ownership claim (not skip, not delete-and-reinsert) — Commit 2 then completes the row |
| Pending-row repair claims a row owned by a concurrent or resumed job instead of the stale job | Low | High | Repair UPDATE predicate is `WHERE id=:stale_doc_id AND ingestion_job_id=:stale_job_id` — it only claims the row if it is still owned by the stale job; `SELECT FOR UPDATE` serializes concurrent repair attempts so only one job can read and claim the row |
| Two concurrent fresh jobs race to claim the same new file version — no existing row to lock with SELECT FOR UPDATE | Low | High | Commit 1 uses `INSERT ... ON CONFLICT DO NOTHING RETURNING id` as the atomic first-claim step; only one INSERT can win; the loser gets no row, reads the winner's row via SELECT FOR UPDATE, finds it pending with the winner's job_id, and aborts cleanly — no work is wasted past Commit 1 |
| Pending-row repair uses DELETE-then-INSERT of the same unique key, failing or behaving unexpectedly under non-deferrable constraint | Low | High | Repair uses UPDATE-in-place (`SET ingestion_job_id=:current_job_id`) instead; the unique key is never deleted and reinserted; integration test against real schema proves correctness |
...
| Any authenticated user triggers unbounded ingestion and Claude API calls | Medium | High | `require_admin` dependency on all ingestion endpoints; role loaded from database on every request, not from JWT claim |
| Revoked admin continues to trigger ingestion until JWT expiry because role is embedded in token | Low | High | JWT payload contains only `user_id`; role is read from the database on every request by `get_current_user`; access token TTL is 15 minutes |
